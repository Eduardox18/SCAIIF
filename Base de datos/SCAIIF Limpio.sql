...
-- Table `SCAIIF`.`estado`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `SCAIIF`.`estado` (
...
CREATE TABLE IF NOT EXISTS `SCAIIF`.`actividad` (
  ...
  INDEX `fk_actividad_estado1_idx` (`idEstado` ASC),
  ...
  CONSTRAINT `fk_actividad_estado1`
    ...
    REFERENCES `SCAIIF`.`estado` (`idEstado`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `SCAIIF`.`observacion` (
  ...
  PRIMARY KEY (`idObservacion`, `matricula`, `noPersonal`),
